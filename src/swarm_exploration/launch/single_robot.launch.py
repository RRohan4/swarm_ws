...
TurtleBot3 Waffle in Gazebo Harmonic (headless) with:
  ...
  - Teleop keyboard (drive with WASD/arrow keys in the terminal)
...
def generate_launch_description():
    ...
    robot_xacro  = os.path.join(tb3_dir, 'urdf', 'gz_waffle.sdf.xacro')
    robot_urdf   = os.path.join(tb3_dir, 'urdf', 'turtlebot3_waffle.urdf')
    bridge_yaml  = os.path.join(tb3_dir, 'configs', 'turtlebot3_waffle_bridge.yaml')

    with open(robot_urdf) as f:
        urdf_content = f.read()

    # Long Gazebo depth camera topic prefix
    depth_prefix = ('/world/swarm_world/model/turtlebot3_waffle'
                    '/link/camera_link/sensor/intel_realsense_r200_depth')
    ...
    return LaunchDescription([
        ...
        AppendEnvironmentVariable('GZ_SIM_RESOURCE_PATH', os.path.join(tb3_dir, 'models')),
        AppendEnvironmentVariable('GZ_SIM_RESOURCE_PATH', str(Path(tb3_dir).parent.resolve())),
        ...
        TimerAction(period=5.0, actions=[
            ...
            Node(
                ...
                arguments=[
                    ...
                    '-string', Command([FindExecutable(name='xacro'), ' ', robot_xacro]),
                    '-x', '0', '-y', '0', '-z', '0.01',
                ],
                output='screen',
            ),
